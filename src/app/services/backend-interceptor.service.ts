        ...
        switch(request.method) {
            case 'GET':
                if (request.url === "/cowData") {
                    return of(new HttpResponse({ status: 200, body: cowData }));
                }
            case 'PUT':
                if (request.url === "/cowData") {
                    cowData.result.map(item => {
				...
				case 'POST':
					if (request.url === "/deleteCowData") {
						const deletedIndex = cowData.result.findIndex(item => item === request.body);
						cowData.result.splice(deletedIndex, 1);
						return of(new HttpResponse({status: 200, body: cowData}));
					}
					if (request.url === "/addCowData") {
						cowData.result.unshift(request.body);
						return of(new HttpResponse({status: 200, body: cowData}));;
					}
        }
        next.handle(request)
